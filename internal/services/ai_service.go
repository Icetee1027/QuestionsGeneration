...
func buildPrompt(request *models.GenerateRequest) string {
	...
	switch request.QuestionType {
	case "單選題":
		promptTemplate = `請以%s為主題，出一個%s難度的單選題。
        請包含四個選項和正確答案。
	...
	case "多選題":
		promptTemplate = `請以%s為主題，出一個%s難度的多選題。
        請包含四個選項和正確答案（可能有多個）。
        回應格式必須完全符合以下 JSON 結構：
        {
            "question_type": "多選題",
	...
	case "是非題":
		promptTemplate = `請以%s為主題，出一個%s難度的是非題。
        請包含正確答案。
        回應格式必須完全符合以下 JSON 結構：
        {
            "question_type": "是非題",
	...
	case "填空題":
		promptTemplate = `請以%s為主題，出一個%s難度的填空題。
	...
	case "簡答題":
		promptTemplate = `請以%s為主題，出一個%s難度的簡答題。
        回應格式必須完全符合以下 JSON 結構：
        {
            "question_type": "簡答題",
            "question": "題目內容",
	...
	case "配對題":
		promptTemplate = `請以%s為主題，出一個%s難度的配對題。
        請提供三組配對項目。
        回應格式必須完全符合以下 JSON 結構：
        {
            "question_type": "配對題",
	...
	case "閱讀題組":
		promptTemplate = `請以%s為主題，出一個%s難度的閱讀題組。
        請包含一篇短文和兩個相關問題。
        回應格式必須完全符合以下 JSON 結構：
        {
            "question_type": "閱讀題組",
	...
	return fmt.Sprintf(promptTemplate, request.Subject, request.Difficulty)
}
